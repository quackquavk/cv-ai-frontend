...
async function generateSitemap() {
  ...
  const staticUrls = [
    ...
    {
      url: `${baseUrl}/user/setting/api-keys`,
      priority: "0.3",
      changefreq: "monthly",
    },
    {
      url: `${baseUrl}/user/setting/appearance`,
      priority: "0.3",
      changefreq: "monthly",
    },
    {
      url: `${baseUrl}/user/setting/billing`,
      priority: "0.3",
      changefreq: "monthly",
    },
    {
      url: `${baseUrl}/user/setting/payment`,
      priority: "0.3",
      changefreq: "monthly",
    },
  ];

  // SEO: Import 1000+ search queries from centralized keywords database
  // Each creates a unique indexable URL: /dashboard?q=react+developer
  const searchQueries = getAllKeywords();
  const keywordStats = getKeywordStats();

  // Add search query URLs to static URLs
  searchQueries.forEach((query) => {
    staticUrls.push({
      url: `${baseUrl}/dashboard?q=${encodeURIComponent(query)}`,
      priority: "0.7",
      changefreq: "daily",
    });
  });

  console.log(`📝 SEO Keywords Statistics:`);
  console.log(`   📊 Total keywords: ${keywordStats.total}`);
  Object.entries(keywordStats.byCategory).forEach(([category, count]) => {
    console.log(`   • ${category}: ${count}`);
  });
  console.log(`   🔗 Search query URLs added: ${searchQueries.length}`);
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`;
  ...
  try {
    console.log(
      `🔗 Attempting to connect to API: ${axiosInstance.defaults.baseURL}`
    );
    ...
    if (documents.length === 0) {
      console.log(
        "⚠️  No documents returned from API - this will result in a static-only sitemap"
      );
      ...
      const batchPromises = batch.map(async (doc, index) => {
        try {
          ...
          const cvResponse = await Promise.race([
            axiosInstance.get(`/document/seo/${doc.doc_id}`),
            timeoutPromise,
          ]);
          ...
          failureCount++;
          console.error(
            `❌ Failed to process document ${doc.doc_id}:`,
            error.message
          );
        }
        return null;
  ...
  <url>
    ...
    <priority>${priority}</priority>
      ...
      const percentage = Math.round(
        (processed / limitedDocuments.length) * 100
      );

      if (i % 60 === 0 || processed >= limitedDocuments.length) {
        console.log(
          `📊 Progress: ${processed}/${limitedDocuments.length} (${percentage}%) | Success: ${successCount} | Failed: ${failureCount}`
        );
      }
    ...
    console.log(
      `   ✅ Success rate: ${Math.round(
        (successCount / limitedDocuments.length) * 100
      )}%`
    );
  } catch (error) {
    ...
    if (!process.env.NEXT_PUBLIC_API_BASE_URL && !process.env.API_BASE_URL) {
      ...
      console.log(
        "   Set NEXT_PUBLIC_API_BASE_URL or API_BASE_URL environment variable"
      );
      console.log(
        '   Example: export NEXT_PUBLIC_API_BASE_URL="https://your-api-url.com"'
      );
      console.log("");
    }
    ...
    console.log(
      "⚠️  WARNING: This sitemap will NOT include any CV detail pages!"
    );
  }
